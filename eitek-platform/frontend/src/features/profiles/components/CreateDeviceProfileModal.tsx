...
export const CreateDeviceProfileModal: React.FC = () => {
  const { isCreateDeviceProfileModalOpen, closeAllModals, createDeviceProfile, saving, error } = useProfileStore();

  const [formData, setFormData] = useState<DeviceProfileCreateRequest>({
    name: '',
    description: '',
  ...
  }, [isCreateDeviceProfileModalOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const submitData: DeviceProfileCreateRequest = {
      name: formData.name.trim(),
      transportType: formData.transportType || 'DEFAULT',
  ...
  if (!isCreateDeviceProfileModalOpen) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-semibold text-gray-900">Create Device Profile</h2>
